...
var jade_mixins = {};
var jade_interp;
;var locals_for_with = (locals || {});(function (static_url, img_url, noavatar_url, nophoto_url, noposter_url, js_url, css_url, IN_PRODUCTION, location_names, quality_texts, months, date_now, Date, undefined, parseInt, Math, res, min, start_date, generic_title, title, title_orig, pg_rating, poster_url, url, watching_users_count, curr_date, start_in_minutes, console, end_date, start_date_class, is_online, has_free, min_price, price_loc, btn_cls, price_loc_cnt, i, obj, url_loc, url_btn, btn_text, title_alt, time_to_wait_str, status_details_text, status_label_text, status_label_class, text, cast) {
static_url = "/static/"
...
console.log("start_in_minutes", start_in_minutes)
console.log('item.start_in', item.start_in)
...
status_label_class = "label-primary"
if( item.start_in < 86400 ) {
start_in_minutes = Math.floor(item.start_in / 60)
status_label_text = ( start_in_minutes < 60 ? 'примерно ' : '' ) + 'через ' + duration_text( start_in_minutes )
} else {
status_label_text += start_date.getDate() + ' ' +  months[start_date.getMonth()]
status_label_text += ' в ' + start_date.getHours() + ':' + ( start_date.getMinutes() < 10 ? '0' : '' ) + start_date.getMinutes()
}
} else if( curr_date < end_date ) {
{
status_label_class = "label-success"
if( watching_users_count !== null && watching_users_count !== undefined ) {
status_details_text = 'смотрят: ' + watching_users_count
}
