...
(function() {
  'use strict';
  var App, CommentThumb, FeedThumb, FilmThumb, Item, Location, Page, Page_Film, Page_Login, Page_Main, Page_Person, Page_Registration, Page_User, PersonThumb, User, check_app_is_init, error,
  ...
  FilmThumb = (function(_super) {
    ...
    function FilmThumb(opts, callback) {
      ...
      FilmThumb.__super__.constructor.call(this, opts, (function(_this) {
        return function() {
          ...
          $(".notinstock button.subscribe-btn", _this._place).click(function() {
            return _this.action_toggle_subscribe();
          });
          $(".playlist-btn", _this._place).click(function() {
            return _this.action_toggle_playlist();
          });
          if (callback) {
            return callback(_this);
          }
    ...
    FilmThumb.prototype.action_toggle_playlist = function(status) {
      ...
      if (this.user_is_auth()) {
        ...
        return this._app.rest.films.action.playlist[action](this.vals("id")).done(function() {
          return rel.playlist = new_playlist;
        });
      }
    };
    ...
    FilmThumb.prototype.action_toggle_notwatch = function(status) {
      ...
      if (this.user_is_auth()) {
        ...
        return this._app.rest.films.action.notwatch[action](this.vals("id")).done(function() {
          return rel.notwatch = new_notwatch;
        });
      }
    };
    ...
    FilmThumb.prototype.set_vals = function() {
      ...
      this._place.attr("id", this.name + "-" + this._vals.id);
      ...
      if (this._vals.hasFree || this._vals.price) {
        ...
        if (this._vals.hasFree) {
          ...
          return $("button.watchbtn", instock).html("СМОТРЕТЬ <br/><span>от <span class=\"price\">" + this._vals.price + "</span> р. без рекламы</span>");
        }
      } else {
  ...
  FeedThumb = (function(_super) {
    ...
    function FeedThumb(opts, callback) {
      ...
      FeedThumb.__super__.constructor.call(this, opts, (function(_this) {
        return function() {
          ...
          return $(".record", _this._place).each(function() {
            var el;
            el = $(this);
    ...
    FeedThumb.prototype.reset = function() {
      return this._vals = {
        id: null,
        user: null,
        object: null
    ...
    FeedThumb.prototype.set_vals = function() {
      FeedThumb.__super__.set_vals.apply(this, arguments);
      if (this._vals._do_not_set) {
        return;
      }
      this._place.attr("id", this._name + "-" + this._vals.id);
      $(".time", this._place).text(time_text(new Date(this._vals.created)));
      if (this._vals.type === "film-o") {
        $(".image", this._place).background_image(this._vals.object.poster || this._app.config("noposter_url"));
        return $(".cont-place a", this._place).attr("href", "/films/" + this._vals.object.id);
      } else if (this._vals.type === "pers-o") {
        $(".image", this._place).background_image(this._vals.object.photo || this._app.config("noperson_url"));
        $(".film-info a", this._place).attr("href", "/films/" + this._vals.object.film.id).text(this._vals.object.film.name);
        return $(".type-" + this._vals.type, this._place).removeClass("display-none");
      } else if (this._vals.type === "sys-a") {

      } else {
        console.log(this._vals.user.name);
        $(".image", this._place).background_image(this._vals.user.avatar || this._app.config("noavatar_url"));
        $(".info-place .name a", this._place).attr("href", "/users/" + this._vals.user.id).text(this._vals.user.name);
        if (this._vals.type.substr(0, 4) === "film") {
          console.log(this._vals.object);
          $(".film-info a", this._place).attr("href", "/films/" + this._vals.object.id).text(this._vals.object.name);
          if (this._vals.type === "film-c") {
            return $(".text", this._place).text(this._vals.object.text);
          } else if (this._vals.type === "film-r") {
            return console.log(1.);
          }
        } else if (this._vals.type === "pers-s") {
          $(".photo", this._place).background_image(this._vals.object.photo || this._app.config("noperson_url"));
          return $(".person-info a", this._place).attr("href", "/persons/" + this._vals.object.id).text(this._vals.object.name);
        } else if (this._vals.type.substr(0, 4) === "user") {
          $(".avatar", this._place).background_image(this._vals.object.avatar || this._app.config("noavatar_url"));
          return $(".cont-place .name a", this._place).attr("href", "/users/" + this._vals.object.id).text(this._vals.object.name);
        }
      }
    };
  ...
  Page_Main = (function(_super) {
    ...
    function Page_Main() {
      ...
      this._e.loadmore = {
        ...
        btn: $("#films_more_btn").click((function(_this) {
          return function() {
      ...
      $(".film-thumb", this._e.films).each(function() {
  ...
  Page_Film = (function(_super) {
    var actors, current_location, films, locations, opts;
    ...
    function Page_Film(new_opts) {
      ...
      this._e.films = $("#films");
      $(".film-thumb", this._e.films).each(function() {
        return films.push(new FilmThumb({
          place: $(this)
        }));
      });
  ...
  Page_User = (function(_super) {
    var _actors, _directors, _feed, _feed_page, _films_subscribed, _films_subscribed_page, _friends, _opts;
    ...
    function Page_User(opts) {
      ...
      this._e.feed = $("#feed_place");
      this._e.more_feed = {
        place: $("#more_feed_place"),
        btn: $("#more_feed_btn").click((function(_this) {
          return function() {
            return _this.load_more_feed();
          };
        })(this))
      };
      this._app.get_tpl("film-thumb");
      ...
      $(".feed-thumb", this._e.feed).each(function() {
        return _feed.push(new FeedThumb({
          place: $(this),
          vals: {
            _do_not_set: true
          }
    ...
    Page_User.prototype.load_more_directors_fav = function() {
      this._e.more_directors_fav.place.hide();
      return this._app.rest.users.persons.read(_opts.id, {
        type: "d"
      }).done((function(_this) {
        return function(data) {
    ...
    Page_User.prototype.load_more_feed = function() {
      this._e.more_feed.place.hide();
      _feed_page++;
      return this._app.rest.users.feed.read(_opts.id, {
        per_page: 5,
        page: _feed_page
      }).done((function(_this) {
        return function(data) {
          ...
          if (data && data.length) {
            for (_i = 0, _len = data.length; _i < _len; _i++) {
              item = data[_i];
              _feed.push(new FeedThumb({
                parent: _this._e.feed,
                vals: item
              }));
            }
            if (data.length >= 5) {
              return _this._e.more_feed.place.show();
            }
          }
        };
      })(this));
