...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'Films', fields ['rating_sort']
        db.create_index('films', ['rating_sort'])

        # Adding index on 'Films', fields ['name_orig']
        db.create_index('films', ['name_orig'])

        # Adding index on 'Films', fields ['type']
        db.create_index('films', ['type'])

        # Adding index on 'Films', fields ['rating_local']
        db.create_index('films', ['rating_local'])

        # Adding index on 'Films', fields ['rating_cons_cnt']
        db.create_index('films', ['rating_cons_cnt'])

        # Adding index on 'Films', fields ['name']
        db.create_index('films', ['name'])

        # Adding index on 'Films', fields ['kinopoisk_id']
        db.create_index('films', ['kinopoisk_id'])

        # Adding index on 'Films', fields ['age_limit']
        db.create_index('films', ['age_limit'])


        # Changing field 'Films.release_date'
        db.alter_column('films', 'release_date', self.gf('django.db.models.fields.DateField')(null=True))
        # Adding index on 'Films', fields ['release_date']
        db.create_index('films', ['release_date'])

        # Adding index on 'Films', fields ['rating_local_cnt']
        db.create_index('films', ['rating_local_cnt'])

        # Adding index on 'Persons', fields ['name']
        db.create_index('persons', ['name'])

        # Adding index on 'Persons', fields ['name_orig']
        db.create_index('persons', ['name_orig'])

        # Adding index on 'Cities', fields ['name']
        db.create_index('cities', ['name'])

        # Adding index on 'Countries', fields ['name']
        db.create_index('countries', ['name'])

        # Adding index on 'Genres', fields ['name']
        db.create_index('genres', ['name'])

        # Adding index on 'FilmExtras', fields ['type']
        db.create_index('films_extras', ['type'])

        # Adding index on 'PersonsExtras', fields ['etype']
        db.create_index('persons_extras', ['etype'])

        # Adding index on 'PersonsFilms', fields ['p_type']
        db.create_index('persons_films', ['p_type'])


    def backwards(self, orm):
        # Removing index on 'PersonsFilms', fields ['p_type']
        db.delete_index('persons_films', ['p_type'])

        # Removing index on 'PersonsExtras', fields ['etype']
        db.delete_index('persons_extras', ['etype'])

        # Removing index on 'FilmExtras', fields ['type']
        db.delete_index('films_extras', ['type'])

        # Removing index on 'Genres', fields ['name']
        db.delete_index('genres', ['name'])

        # Removing index on 'Countries', fields ['name']
        db.delete_index('countries', ['name'])

        # Removing index on 'Cities', fields ['name']
        db.delete_index('cities', ['name'])

        # Removing index on 'Persons', fields ['name_orig']
        db.delete_index('persons', ['name_orig'])

        # Removing index on 'Persons', fields ['name']
        db.delete_index('persons', ['name'])

        # Removing index on 'Films', fields ['rating_local_cnt']
        db.delete_index('films', ['rating_local_cnt'])

        # Removing index on 'Films', fields ['release_date']
        db.delete_index('films', ['release_date'])

        # Removing index on 'Films', fields ['age_limit']
        db.delete_index('films', ['age_limit'])

        # Removing index on 'Films', fields ['kinopoisk_id']
        db.delete_index('films', ['kinopoisk_id'])

        # Removing index on 'Films', fields ['name']
        db.delete_index('films', ['name'])

        # Removing index on 'Films', fields ['rating_cons_cnt']
        db.delete_index('films', ['rating_cons_cnt'])

        # Removing index on 'Films', fields ['rating_local']
        db.delete_index('films', ['rating_local'])

        # Removing index on 'Films', fields ['type']
        db.delete_index('films', ['type'])

        # Removing index on 'Films', fields ['name_orig']
        db.delete_index('films', ['name_orig'])

        # Removing index on 'Films', fields ['rating_sort']
        db.delete_index('films', ['rating_sort'])


        # Changing field 'Films.release_date'
        db.alter_column('films', 'release_date', self.gf('django.db.models.fields.DateField')(default=''))
    ...
    models = {
        ...
        'films.films': {
            ...
            'rating_local_cnt': ('django.db.models.fields.PositiveSmallIntegerField', [], {'default': '0', 'null': 'True', 'db_index': 'True', 'blank': 'True'}),
