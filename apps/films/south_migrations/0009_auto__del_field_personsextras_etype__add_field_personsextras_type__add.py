...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'PersonsExtras.etype'
        db.delete_column('persons_extras', 'etype')

        # Adding field 'PersonsExtras.type'
        db.add_column('persons_extras', 'type',
                      self.gf('django.db.models.fields.CharField')(max_length=255, db_index=True),
                      keep_default=False)

        # Adding index on 'PersonsFilms', fields ['p_index']
        db.create_index('persons_films', ['p_index'])

        # Create table view
        sql = """
        CREATE VIEW first_3_actor_by_film AS (
            SELECT a.*
            FROM persons_films AS a
            WHERE a.p_type='actor' and a.id IN (
                SELECT b.id
                FROM persons_films AS b
                WHERE b.film_id = a.film_id and b.p_type='actor'
                ORDER BY b.p_index ASC
                LIMIT 3)
            ORDER BY a.film_id ASC, a.p_index ASC)
        """
    ...
    def backwards(self, orm):
        # Removing index on 'PersonsFilms', fields ['p_index']
        db.delete_index('persons_films', ['p_index'])
        ...
        # The following code is provided here to aid in writing a correct migration        # Adding field 'PersonsExtras.etype'
        db.add_column('persons_extras', 'etype',
                      self.gf('django.db.models.fields.CharField')(max_length=255, db_index=True),
                      keep_default=False)

        # Deleting field 'PersonsExtras.type'
        db.delete_column('persons_extras', 'type')

        # Drop table view
        sql = "DROP VIEW first_3_actor_by_film"
        db.execute(sql)
    ...
    models = {
        ...
        'films.personsfilms': {
            ...
            'p_index': ('django.db.models.fields.IntegerField', [], {'default': '0', 'db_index': 'True'}),
