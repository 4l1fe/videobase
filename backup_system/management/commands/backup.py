...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            backup_file = APP_BACKUP_FILENAME_TEMPLATE.\
                format(db_name=db_name, time=str(time.strftime("%Y-%m-%d-%H-%M")))

            command = "PGPASSWORD={passwd} pg_dump {name} --username {user} --host={host} --port={port} -F c > {filename} ".\
                format(passwd=db_pass, name=db_name, user=db_user, host=db_host,
                       port=db_port, filename=os.path.join(path, backup_file))

            p = subprocess.Popen(command, shell=True).wait()
            logger.info("End: Backup for DATABASE: {0}".format(db_name))
        except Exception as e:
            logger.error("End: Backup for DATABASE:{0} error: {1}".format(db_name, e))
